...
def extract_features(event_log):
    ...
    print(case_durations)  # Adicione essa linha para verificar a estrutura de case_durations
    
    if isinstance(case_durations, list) and all(isinstance(case, dict) for case in case_durations):
        case_durations_dict = {case['case']: case['case_duration'] for case in case_durations}
    else:
        raise ValueError("Estrutura de case_durations inesperada.")
    ...
    for trace in event_log:
        ...
        case_duration = case_durations_dict[trace_id]
...
def simulate_process(event_log, svm_model, num_simulations=5):
    for i in range(num_simulations):
        print(f'Simulação {i + 1} de {num_simulations}')
...
print('Log de eventos carregado com sucesso.')
...
print('Log simulado salvo com sucesso.')
